...
| **Value Search — by-value scan** *(build 738 + Phase 2 757)* | CE-style **First Scan / Next Scan** workflow over every UPROPERTY-declared field. **Numeric primitives** (Int8/16/32/64 + UInt + Float / Double / Bool) with Exact / Bigger / Smaller / Between / Changed / Unchanged / Increased / Decreased + CE-style rounded float tolerance. **Phase 2A strings** (FString / FName / FText) with Contains / StartsWith / EndsWith and optional case-sensitive matching. **Phase 2B vectors** (FVector / FRotator) with `"X,Y,Z"` CSV input and per-axis tolerance. **Phase 2C TArray\<T\>** element scan emits `FieldName[N]` candidates with a 10 M-element soft circuit-breaker. Native C++ fields (non-UPROPERTY) are explicitly out of scope — banner directs users to CE's raw memory scan. Sessions auto-expire after 5 min. |
...
| **Modern UI** | External **Avalonia UI** connected via high-speed Named Pipe (~42 JSON-RPC commands). Native AOT published with zero IL2026 / IL3050 warnings. Keep your game performance smooth. |
...
**License**: [GPL-3.0](LICENSE) © 2026 bbfox0703