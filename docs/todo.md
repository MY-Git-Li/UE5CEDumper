...
- **P4 increment 2 — per-slot `owner_class` for the Pivot handoff** — Effort: **S** · Risk: low. **(Increment 1 DONE + MERGED main PR #313 `d977a34`, builds 1303-1313; in-game VERIFIED on TQ2 GAS — see dev-log 2026-06-18 + [group-value-scan-spec.md](group-value-scan-spec.md) §3.2 P4.)** The cross-object handoffs that open an OBJECT (Live Walker / Locate-in-GWorld) already target the owned sub-object via `GroupSlotMatch.HandoffAddr` (= `owner_addr`). The remaining gap is the **Pivot** handoff (`PivotGroupSlot`): it uses `GroupSlotMatch.ClassName`, which `ParseGroupCandidate` denormalizes from the **candidate actor**, so for a cross-object slot it pivots on the actor's class instead of the owned sub-object's (e.g. the stats component / `UAttributeSet`). **To do:** DLL `GroupCandidateToJson` — emit a per-slot `owner_class` (the leaf's owning object class; for an own-block leaf == the candidate class). The DLL already has it via the leaf's `definingClassName`, OR add an `ownerClass` to `Radar::GroupLeafMeta`/`GroupSlotMatch` (mirror `ownerAddr`) and resolve `Ubel::GetName(Ubel::GetClass(ownerAddr))` at emit. C# `GroupSlotMatch.OwnerClass` (parse `owner_class`); `PivotGroupSlot` prefers `OwnerClass ?? ClassName`. Small + isolated. *Parent: P4 inc1 handoffs target the sub-object by ADDRESS but not by CLASS.*
