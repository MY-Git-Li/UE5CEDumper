...
- **Snapshot Group Match — S1-S5 BUILT on `dev` (builds 1563-1569); verify only.** N-value group matching over the captured-snapshot corpus via a pure-C# `Orden` port (`Services/GroupMatch.cs`). **Mode A** = single-snapshot absolute find; **Mode B** = cross-snapshot temporal comparison (per-slot `Changed/Unchanged/Increased/Decreased` incl. **`Unchanged`** — the "Current HP↓ + Max HP unchanged" group); **S5 Deep** = opt-in "Deep (nested containers)" toggle that folds nested array / struct-array element values (`…Tunes[2]`) into the owning object's block (the SEED Tunes case — driven by the first in-game test). UI = Snapshot panel Diff/Group toggle + a "Compare with" snapshot picker + Deep checkbox. Unit-tested (matcher SDR/width/scope, Mode A + B store, VM wiring); adversarial 4-lens review → 6 findings fixed (`aad2ed7`). Full design + status: [snapshot-group-match-spec.md](snapshot-group-match-spec.md); dev-log 2026-06-23. **Remaining (verify only):** in-game — capture two snapshots around a stat change, Group mode + Compare with → confirm the HP-pair case. **Known limitation (deferred):** integer values > 2^53 lose precision (snapshot-wide double compare, shared with SPC/Diff/Pivot — a deep 64-bit-exact refactor, not unique to group match). **Still deferred (same matcher, later):** SPC Query "N-field intersection" + Class Pivot "co-varying tuples" (spec §3.1 rows 2-3). *Object-flat Deep is DONE (S5); array-AS-BLOCK (each array its own block, like the live deep) remains an optional refinement.* *Parent: spec written 2026-06-22; S1-S5 built 2026-06-23.*
