...
- **P4 — cross-object actor block (owned sub-objects), opt-in** — **Increment 1 SHIPPED (builds 1303-1309, dev; ⚠ in-game verify pending).** Approach C (ownership + value-driven, NOT class-name matching) — full design in [group-value-scan-spec.md](group-value-scan-spec.md) §3.2 P4, dev-log 2026-06-18. `Aura::AppendOwnedSubObjectLeaves` runs a bounded 2-level owned BFS (`EnumerateOutgoingObjectPtrs` + `IsOwnedBy` Outer-chain gate) folding each owned sub-object's numeric leaves into the actor's block; depth 1 = components, depth 2 = GAS AttributeSets via the ASC's `SpawnedAttributes`. Pipe `cross_object` flag + per-slot `owner_addr`; C# `CrossObjectScan` checkbox + `HandoffAddr` (handoffs open the owning sub-object). refine/Orden/session unchanged (leafAddr-keyed). **Remaining — increment 2 (Effort S):** per-slot `owner_class` so the **Pivot** handoff pivots on the sub-object's class (today it uses the actor's class); + in-game verify on a game whose values genuinely split actor ↔ component/AttributeSet (SEED's live on the save object — needs a GAS or component-based title). *Parent: P1 finds co-located stats; P4 adds the cross-object-distributed case.*
