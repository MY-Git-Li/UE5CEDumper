...
- **Snapshot Group Match — DESIGNED (spec written), not built. Effort S1-S3 ≈ M-L; S4 Mode B ≈ M-L.** Bring N-value group matching to the captured-snapshot corpus via a **C# `Orden` port** (the source-agnostic SDR seam). Full design of record: [snapshot-group-match-spec.md](snapshot-group-match-spec.md). **Mode A** = single-snapshot absolute find (Exact/Bigger/Smaller/Between); **Mode B** = cross-snapshot temporal comparison — per-slot `Changed/Unchanged/Increased/Decreased`. **Key design decision (user, 2026-06-22): groups MUST include `Unchanged`** — a multi-value group commonly has unchanged members (Current HP↓ + Max HP unchanged), so forcing every slot to a moving predicate would miss the group; single-value `Unchanged` stays in SPC, single-value Diff stays changes-only. **Decisions LOCKED (2026-06-22): host = Snapshot panel Diff/Group toggle; matcher = C# `Orden` port; v1 = Mode B (≥2-snapshot comparison), NOT single-snapshot** — a single snapshot is a frozen instant (shows absolute values but no change), so it can't express the motivating case; Mode A (single-snapshot absolute) is just the degenerate 1-snapshot path of the same engine. Phases S1 (C# Orden port + per-slot predicate eval, absolute + relative) → S2 (`GroupMatchEngine`/`SnapshotStore.GroupMatchAsync` over the SPC ≥2-snapshot intersection = Mode B; Mode A degenerate) → S3 (Snapshot panel Diff/Group toggle, ≥2-snapshot picker, per-slot predicate column, extract shared input+master-detail control) → S4 (deep blocks, optional). **Still deferred: SPC Query "N-field intersection" + Class Pivot "co-varying tuples"** (spec §3.1 rows 2-3) — same matcher, later. *Parent: P1 kept the matcher live-scan-agnostic for this; spec written 2026-06-22.*
