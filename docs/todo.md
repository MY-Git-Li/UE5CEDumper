...
## Land `GWLD_FD_1` — a mined, measured GWorld pattern awaiting only a full sweep

*Parent: the UE 4.11/4.13 support pass (dev-log 2026-07-27, builds 2464-2476). Mined after the
maintainer declined to tighten `ValidateGWorld` for one old title — "add a pattern, don't touch a
validator 30+ oracles depend on".* **Effort S · Risk low.**

Everything below is measured over **45 program instances / 23 GWorld oracles, UE 4.11 → 5.7**.
Nothing has been committed; the only outstanding work is the full `sweep.sh` +
`aggregate_sweep.py` the header demands before any pattern change.

```
48 8B 05 ?? ?? ?? ?? 48 3B C? 48 0F 44 C? 48 89 05 ?? ?? ?? ?? E8
```

`instrOffset 0 · opcodeLen 3 · totalLen 7 · adjustment 0 · gworldAllowNull false`
21 bytes, **12 fully-literal** + 2 nibble-fixed. Source: `UWorld::FinishDestroy`, confirmed from
PDB symbols on **three independent oracles** (HeliumRain 4.20, DropIn 4.24, DropIn 4.27).

```asm
mov   rax,[rip+GWorld]     48 8B 05 d32   <- resolved here (io = 0)
cmp   rax,<this>           48 3B C?       C6 on 4.11, C7 on 4.13, C3 on 4.20-5.2
cmovz rax,<replacement>    48 0F 44 C?
mov   [rip+GWorld],rax     48 89 05 d32   <- SAME global, written back
call  ...                  E8
```

Both nibble masks pin the operand to `rax`, tying the compare and the cmov to the value just
loaded. **A read followed by a conditional write-back of the same global is self-evidencing** —
that is why it is clean rather than merely long.

**21 hits, 16 correct, ZERO decoys anywhere**, never more than 1 hit on any binary (≤0.1 hits/MB).

| verdict | where |
|---|---|
| UNIQUE-OK (16) | 4.11, 4.13, 4.18 DQ XI S, 4.20 ×2, 4.21, 4.24, 4.25, 4.26, 4.27 ×3, 5.0, 5.1, 5.2 |
| NO-TRUTH, 1 hit (5) | FF7 Remake, Octopath, Artisan, Palworld, DQ I&II |
| MISS (24) | 4.22, FF7 Rebirth, Avowed, Elliot, all 5.5/5.6, Solarpunk |

### Why it is worth landing beyond the two old games

- It is a **strict generalisation of `GWLD_G427_5`** (same function). The nibbled CMP reaches the
  rsi/rdi codegen, the exact `48 0F 44 C?` tightens a CMOV `G427_5` leaves wildcarded, and the
  trailing `E8` adds a literal byte. **`G427_5` is DECOY-ONLY on both 4.11 and 4.13.**
- **Fantasynth 4.13 also mis-selects today**, via `GWLD_SF_2` at priority 300 — *earlier* in the
  walk than the NEKOPALIVE decoy. So this fixes two binaries, not one.
- Decoy check passed explicitly: it does **not** hit NEKOPALIVE's `0x1423C9940` TSharedPtr
  singleton, nor Fantasynth's analogues (`14288E648`, `1427B0B40`, `14277D3C0`).

### The one open decision — 265 vs Tier-1

`GWORLD_PATTERNS` holds 49 byte patterns = 6 full batches + 1; adding one or two stays at 7, so
**no batch boundary is crossed either way**.

- **265** (between `GWLD_GH_3` 260 and `GWLD_SF_1` 270) is **provably free**: batches 1-2 stay
  byte-identical so nothing landing on the modern Tier-1 block can be perturbed, and it sits in
  batch 3 *ahead of* `SF_2` (300), which is what fixes Fantasynth. 300+ would fix NEKOPALIVE but
  **not** Fantasynth.
- **Tier-1 (~102)** is defensible on the data — 16 UNIQUE-OK / 0 decoys is better than
  `GWLD_TQ_1`'s recorded 10 — but the existing table was **not** re-measured corpus-wide, so what
  it would displace is unknown. Take 265 unless someone does that measurement.

### Caveats to carry into the Himmel.h comment

- **No 4.12 binary exists in the corpus.** 4.12 is bracketed structurally (identical shape on 4.11
  and 4.13 and every version through 5.2) but never measured — do not write it up as verified.
- `48 3B C?` also admits `cmp rcx,r??`. It cost zero decoys over 45 programs, but it is looser
  than the intent; note it rather than tighten it blind.
- `instrOffset` was proven by counter-example: setting it to 4 keeps the hit count **identical**
  while the resolved address lands outside the image.

### Held, not recommended

A second candidate on `UEngine::TickDeferredCommands`
(`48 8B 1D ?? ?? ?? ?? 48 8B 75 ?? E8 ?? ?? ?? ?? 4C 8B C8 48 8D 4D ?? 48 8B D3`, 16 literal,
7 UNIQUE-OK, 0 decoys) **adds no oracle the above does not already cover**. Its only value would
be a structurally different pre-4.22 anchor, or as a lengthened replacement for `GWLD_G42_1`
(same site, 7 literal, priority 880) — and a replacement needs `G42_1` measured corpus-wide first,
which was not done. Also rejected: `48 8B 0D ?? ?? ?? ?? 41 B0 01 33 D2` — **8 literal bytes**,
44 hits / 4 correct / 34 decoys, a textbook confirmation of the "<~8 literal ⇒ band 800+" rule.
