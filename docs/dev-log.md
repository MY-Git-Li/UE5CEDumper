...
Tests: **1358 unchanged** (31 utf8 + 247 dll_helpers + 1080 C#). The existing suites stub the DLL / test predicates, so they confirm no compile-or-logic regression but **do not exercise the live parallel walk** — in-game verification (inject into ES2 + a large-heap title; parallel result set must equal serial; First-Scan wall-clock should drop ~cores×) is pending and listed in [todo.md](todo.md).
