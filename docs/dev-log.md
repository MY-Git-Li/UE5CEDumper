...
## 2026-06-23 — Snapshot capture perf: telemetry + overlap + single-connection bulk session + incremental pivot (Phases 0-2; builds 1585-1591; dev; in-game measure PENDING; Phase 3 query-cache deferred)
...
C# 1852/0; Native AOT publish clean (47.5 MB); `Channels` + the session trim-safe. **In-game measurement PENDING:** re-inject the new DLL + run the new UI, capture a big game, read the `walk/ser/parse/write` breakdown + confirm the finalize freeze is gone. **Phase 3 (query row cache + 25%-of-available / 4 GiB memory budget) is deferred** until the measurement confirms where the time goes (the plan's "measure-first" gate). Design of record: the approved plan + the perf-eval workflow.
