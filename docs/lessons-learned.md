...
- **Vtable-index detection for ProcessEvent is unreliable** (discovered build 647). `DetectProcessEventVTableOffset` in `dll/src/Frieren.cpp` picks the slot purely from a hardcoded UE-version table and "validates" by checking that `vtable[offset]` points to readable code -- which every entry does, so the check is a no-op. On both ES2 (UE 5.5, slot 0x220) and Geri (UE 4.27, slot 0x218) the resulting hook landed on **a UObject virtual adjacent to ProcessEvent** (likely `CallFunction` or similar). Symptoms that surfaced live: instance-method invokes always time out at `-5` (game thread is busy firing PE many times/sec but our queue never drains -- hook isn't on PE); static-native fast path "succeeds" with `return=0` because the wrong vtable slot has a compatible-enough ABI to not AV but doesn't write anything useful. **The whole UFunction invoke pipeline depends on getting this offset right.** Replace with AOB scan of the PE prologue (RE-UE4SS / Dumper-7 both have stable patterns) and validate post-install by checking the hook actually fires within ~1s of live gameplay. The silent-failure mode here is why this bug slept for 600+ builds.
...
- **KismetMathLibrary helpers are stubs in cooked Shipping (UE 5.5+)**: `KismetMathLibrary::Exp`, `Multiply_DoubleDouble`, `Add_IntInt`, etc. all have intact reflection metadata (correct flags, parmsSize, NumParms, parm offsets) but their `execXxx` thunks return without writing to the ReturnValueOffset. Likely a side effect of UE's BlueprintFastCall optimisation, where the BP VM bytecode bypasses ProcessEvent entirely and calls the C++ impl directly. Reflection-driven invokers (us, custom dumpers) end up calling the stub. **Don't use KismetMathLibrary as a verification target** — pick a game-specific instance method instead. Verified failing on Everspace 2 (UE 5.5).
