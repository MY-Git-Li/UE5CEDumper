...
|      +-- Frieren  (ExportAPI)     <- 36 C ABI for CE Lua (6 teleport, build 1027) |
|      +-- Fern     (PipeServer)    <- Named Pipe IPC (~48 cmds incl. value scan + teleport) |
...
|      +-- Wirbel   (Teleport)      <- BugIt-style marker save/recall + cursor teleport; CMD_TELEPORT=8 mailbox (build 1027) |
+----------------------+-------------------------------+
...
|      +-- TeleportPanel             <- marker save/recall + cursor teleport + BugItGo + CE Lua bundle (build 1027) |
