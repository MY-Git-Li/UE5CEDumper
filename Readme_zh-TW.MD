...
- **角色調整（Player tuning）** — 用 10%–3000% 滑桿切換 **Super Jump（超級跳）**、**Move Speed（移動速度）** 與 **Gravity（重力）**，外加 **God Mode（無敵）** — 全部以反射在玩家 pawn 上強制設定，並對抗遊戲每 tick 的覆寫，因此重生後仍維持。可綁全域熱鍵，或匯出成 Cheat Engine 的開 / 關記錄（100% = 關閉）。
