...
public class CeMailboxBailoutTests
{
    ...
    public void ATopLevelBusyMailboxBail_UnticksTheRecordAndSaysWhy()
    {
        ...
        var lines = EnableBlock((string)HelperShapedScripts().First()[1]).Split('\n');
    ...
    {
        string enable = EnableBlock((string)HelperShapedScripts().Last()[1]);
    ...
    public void TheIdleWaitBodyIsTheSharedEmittersVerbatim()
    {
        ...
        foreach (var row in HelperShapedScripts())
    ...
    {
        foreach (var row in HelperShapedScripts())
            Assert.Equal(1, Count((string)row[1], "local idleWaited = 0"));
    }
    ...
    {
        string enable = EnableBlock(script);
        int idle = enable.IndexOf("local idleWaited = 0", StringComparison.Ordinal);
